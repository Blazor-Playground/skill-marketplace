...
bootstrapCmd.SetAction(pr =>
{
    ...
    var cloneUrl = GetGitRemoteUrl();
    if (cloneUrl == null)
    {
        PrintError("  No git remote 'origin' found. Run this from a cloned repo or set a remote.");
        return;
    }
...
// ----- Marketplace -----
var marketplaceCmd = new Command("marketplace", "Marketplace authoring utilities");
{
...
void InstallCliInstructions(string[] sourceFiles, bool force, bool dryRun, bool verbose)
{
    ...
    writer.WriteLine("<!-- Auto-generated by skill-marketplace.cs from repo instructions/ -->");
    writer.WriteLine();
...
// ============================================================================
// Marketplace Init Handler
// ============================================================================

void RunMarketplaceInit(bool dryRun, bool verbose)
{
    PrintHeader("Marketplace Init");

    // Prompt for plugin group details
    Console.Write("  Plugin group name (e.g., my-team): ");
    var pluginName = Console.ReadLine()?.Trim();
    if (string.IsNullOrWhiteSpace(pluginName))
    {
        PrintError("  Plugin group name is required.");
        return;
    }

    Console.Write("  Plugin group description: ");
    var pluginDescription = Console.ReadLine()?.Trim();
    if (string.IsNullOrWhiteSpace(pluginDescription))
    {
        PrintError("  Plugin group description is required.");
        return;
    }

    Console.Write("  First skill name (e.g., code-helper): ");
    var skillName = Console.ReadLine()?.Trim();
    ...
    Console.Write("  First skill description: ");
    var skillDescription = Console.ReadLine()?.Trim();
    if (string.IsNullOrWhiteSpace(skillDescription))
    {
        PrintError("  Skill description is required.");
        return;
    }

    // Paths
    var pluginDir = Path.Combine(repoRoot, "plugins", pluginName);
    var skillDir = Path.Combine(pluginDir, "skills", skillName);
    ...
    {
        PrintError($"  Plugin group '{pluginName}' already exists at: {pluginDir}");
        return;
    }

    if (dryRun)
    {
        PrintInfo($"  Would create: {pluginJsonPath}");
        PrintInfo($"  Would create: {skillMdPath}");
        PrintInfo($"  Would update: {marketplacePath}");
        return;
    }
    ...
    WriteJsonNode(pluginJsonPath, pluginJson);
    if (verbose) PrintInfo($"    Created {pluginJsonPath}");
    ...
    var skillMd = $"""
        ...
        """.Replace("        ", ""); // strip leading indent
    File.WriteAllText(skillMdPath, skillMd);
    if (verbose) PrintInfo($"    Created {skillMdPath}");
    ...
    if (File.Exists(marketplacePath))
    {
        ...
        if (root != null)
        {
            ...
            WriteJsonNode(marketplacePath, root);
            if (verbose) PrintInfo($"    Updated {marketplacePath}");
        }
    }
    else
    {
        PrintWarning($"  marketplace.json not found at {marketplacePath} — skipping registration.");
    }

    Console.WriteLine();
    PrintSuccess($"  Created plugin group '{pluginName}' with skill '{skillName}'");
...
string SkillTitleCase(string kebab) =>
    string.Join(" ", kebab.Split('-').Select(w =>
